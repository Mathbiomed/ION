...
function R_score = plot_reg_detect_self_fourier(rcolumn, scolumn, data, ffit_n)
...
load(['Output/Var_' num2str(scolumn) '.mat'], 'fitresults')
...
f_der1 = f_der1 + i*c1(2*i+1)*c1(end)*cos(i*c1(end)*x) - i*c1(2*i)*c1(end)*sin(i*c1(end)*x);
...
f_der2 = f_der2 + i*c2(2*i+1)*c2(end)*cos(i*c2(end)*x) - i*c2(2*i)*c2(end)*sin(i*c2(end)*x);

end


f_der1 = matlabFunction(f_der1);

f_der2 = matlabFunction(f_der2);
...
for i = 1:length(tv)
        ...
        try
            ...
            R = compute_R_self_fourier(data,rcolumn,scolumn,data(i,scolumn), data(i,1), f_der1, f_der2);
            Rs = [Rs; R];
            ts = [ts; data(i,1)];
...
filehead = ['Output/Reg_detect_', num2str(rcolumn-1), '_onto_', num2str(rcolumn-1), '_fix_', num2str(scolumn-1)];
